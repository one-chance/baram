...
export default function AbilityTable() {
  ...
  useEffect(() => {
    const calAbility = () => {
      ...
      for (let idx = 0; idx < 6; idx++) {
        ...
        if (b > c) {
          total[idx] = (Math.floor((a + b + d) * (1 + e / 100)) + f) * g + h;
        } else {
          total[idx] = (Math.floor((a + c + d) * (1 + e / 100)) + f) * g + h;
        }
      }
      setTotalState(total);
    };
  ...
  return (
    <>
      <Grid container direction='row' alignItems='center' justify='center' className={smallScreen ? classes.column : classes.row}>
        <Grid container direction={smallScreen ? "column" : "row"}>

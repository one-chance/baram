...
export const DeletePost = async (_category: CategoryType, _seq: number) => {
  ...
  const res = await axios
    .delete(`/api/board/${_category}/post/${_seq}`, {
      headers: {
        token: CommonUtil.getToken(),
        key: signInUser.key,
        id: signInUser.id,
...
export const RecommendPost = async (_category: CategoryType, _seq: number, _userid: string) => {
  const res = await axios
    .post(
      `/api/board/${_category}/post/recommend/${_seq}`,
      {
        seq: _seq,
        userid: _userid,
      },
...
export const UnrecommendPost = async (_category: CategoryType, _seq: number, _userid: string) => {
  const res = await axios
    .post(
      `/api/board/${_category}/post/unrecommend/${_seq}`,
      {
        seq: _seq,
        userid: _userid,
      },
...
export const EditComment = async (post: IPost, comment: IComment) => {
  ...
  const res = await axios
    .put(
      `/api/board/${post.category}/comment`,
      {
        post: post,
        comment: comment,
      },
...
export const DeleteComment = async (post: IPost, commentIdx: number) => {
  ...
  const res = await axios
    .delete(`/api/board/${post.category}/comment/${post.seq}/${commentIdx}`, {
      headers: {
        token: CommonUtil.getToken(),
        key: signInUser.key,
        id: signInUser.id,
...
export const CreateRecomment = async (post: IPost, comment: IComment, recommentCount: number, recommentMessage: string) => {
  ...
  const res = await axios
    .post(
      `/api/board/${post.category}/recomment`,
      {
        seq: post.seq,
...
export const DeleteRecomment = async (post: IPost, commentIdx: number, recomment: IRecomment) => {
  ...
  const res = await axios
    .put(
      `/api/board/${post.category}/recomment/${recomment.idx}`,
      {
        post: post,
        commentIdx: commentIdx,
        recomment: recomment,
...
export const getPostCount = async (_category: CategoryType, _filterUri?: string) => {
  ...
  await axios
    .get(`/api/board/${_category}/count?${_filterUri}`)
    .then(res => {
      if (res.data.code === 200) {
        if (res.data.count) count = Number(res.data.count);
      }
      return true;
    })
    .catch(e => {
...
export const getPosts = async (_category: CategoryType, _filterUri?: string, _page?: number, _pageSize?: number) => {
  ...
  if (_filterUri !== undefined) _filterUri += `&` + pageQuery;
  else _filterUri = pageQuery;

  await axios
    .get(`/api/board/${_category}/find?${_filterUri}`)
    .then(res => {
      if (res.data.code === 200) {
